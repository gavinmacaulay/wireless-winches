...
class storeValue():
    ...
    def put(self, v):
        ...
        f[self.cPos] = self.__checksum(ba)  # store checksum
        ...
        f[0] = self.cPos  # store location

    def get(self):
        """Retrieve a value."""
        cPos = f[0] # get location of most recently written data
        checksum = f[cPos]
        ba = bytearray(self.storeSize)
        _ = f.readwrite(cPos+1, ba, read=True)
