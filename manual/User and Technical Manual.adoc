...
The winch and controller status messages that are sent to the Android app over Bluetooth are also copied out on the controller's USB connector via a virtual serial port when using controller code dated from June 2025 onwards. This can be useful if an Android device is not available, or the winch status app cannot be installed.

Three basic displays of this information are provided:
