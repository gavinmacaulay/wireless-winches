...
Connect a Monitor device to a USB port, whereupon winch and controller status messages will be output on a virtual serial port. Information on Monitor devices is given in <<Monitor Xbee device>> and the format of the messages they produce are detailed in <<_communication>>. Multiple Monitor devices can be used at the same time.
...
The controller also sends a message to the Android app and to any Monitor Xbee devices using the same message style as the message sent by the winches (see below) and contains information about the controller. The message is a comma separated ASCII string in the form:
...
A Monitor Xbee is a device that receives copies of the winch and controller status messages and outputs them on a USB/serial port. The Monitor can be used to obtain winch status data independent of the Android app. Multiple Monitor devices can operate simultaneously, each outputting the winch and controller status messages. The Sparkfun Thing Plus Xbee is recommended for use as a Monitor device as it inorporates a USB connection that provides both serial data and a power supply for the Xbee. 

Three example winch status display programs are provided:

* https://github.com/gavinmacaulay/wireless-winches/blob/main/code/status_display_gui.py[Python code] that produces a simple GUI display of the winch information
* https://github.com/gavinmacaulay/wireless-winches/blob/main/code/status_display.py[Python code] that produces a terminal-based text display of the winch information
* https://github.com/gavinmacaulay/wireless-winches/blob/main/code/status_display_web.html[HTML code] for web browsers that support the https://developer.mozilla.org/en-US/docs/Web/API/Web_Serial_API[Web Serial API] (mainly Chromium-based ones, such as Chome, Edge, and Opera)

You can also write your own code to display the status messages or use dashboard software that can parse data from a serial port (e.g., https://github.com/Serial-Studio/Serial-Studio[Serial-Studio]).

The virtual serial port should be set to 9600/8/N/1 and is provided by a Silicon Labs CP210x USB to UART bridge device on all controllers that use the Sparkfun Thing Plus Xbee radio unit.
